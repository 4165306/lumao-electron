...
export class Syncswap implements DAppInterface {
  ...
  public async run(fromChain: string, _toChain: string, fromToken: string, toToken: string) {
    ...
    while (true) {
      ...
      await p
        ...
        .catch(() => {})
      // 尝试切换链
      const currentChainImage = await p
        .locator('#navi-tool > div.relative > div > button > div > div:nth-child(1) > div > img')
        .getAttribute('src')
      if (currentChainImage?.toUpperCase().indexOf(chain.toUpperCase()) === -1) {
        // 没选对链，执行选链
        await p
          .locator('//*[@id="navi-tool"]/div[1]/div/button')
          .click({ timeout: 1000 })
          .catch(() => {})
        await p
    ...
    // 选择第一个币
    const currentFromToken = await p
      .locator('#swap-input > div.row > div > div > div.row.align > p')
      .innerText()
    ...
    const currentToToken = await p
      .locator(
        '#swap-box > div:nth-child(1) > div > div.col.gap-10 > div.col.align > div:nth-child(3) > div.row > div > div > div.row.align > p'
      )
      .innerText()
    if (currentToToken.toUpperCase().indexOf(toToken.toUpperCase()) === -1) {
      await p
        .locator(
          '#swap-box > div:nth-child(1) > div > div.col.gap-10 > div.col.align > div:nth-child(3) > div.row > div > div'
        )
        .click()
      await this.selectToken(toToken, p)
    }
    await p.waitForTimeout(1000)
    const balanceData = await p
      .locator(
        '#swap-input > div.col.gap-12 > div:nth-child(1) > div.row.gap-8.align.fade-in-mid > div > p'
      )
      .innerText()
    ...
    if (isGas(fromToken)) {
      ...
      // 输入gas的80%
      await p
        .locator('#swap-input > div.row > input')
        .fill((parseFloat(balance[0]) * 0.8).toFixed(5))
    } else {
  ...
  public async selectNetwork(chain: string, p: Page) {
    p.locator('#navi-tool').getByRole('button').first().click()
    p.getByText(chain).click()
  ...
  public async selectToken(token: string, p: Page) {
    ...
    await p
      ...
      .catch(() => {})
    await p
      .locator('//*[@id="toolbox"]/div/div[1]/div[2]')
      .click({ timeout: 3000 })
      .catch(() => {})
  }
